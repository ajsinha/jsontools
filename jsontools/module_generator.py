...
def _get_class_name(schema: Dict, filename: str) -> str:
    """
    Get class name from the schema filename (not title).
    AssetClass.json -> AssetClass
    01_user.json -> User
    my-schema.json -> MySchema
    """
    # Remove leading numbers and underscore (e.g., 01_user.json -> user.json)
    name = re.sub(r'^\d+_', '', Path(filename).stem)
    # Convert to PascalCase
    class_name = to_pascal_case(name)
    # Remove "Schema" suffix if present (we don't want AssetClassSchema)
    if class_name.endswith("Schema") and len(class_name) > 6:
...
def _get_module_name(schema: Dict, filename: str) -> str:
    """
    Get module name from the schema filename (not title).
    AssetClass.json -> asset_class
    01_user.json -> user
    my-schema.json -> my_schema
    """
    # Remove leading numbers and underscore (e.g., 01_user.json -> user.json)
    name = re.sub(r'^\d+_', '', Path(filename).stem)
    ...
    if module_name.endswith("_schema") and len(module_name) > 7:
        module_name = module_name[:-7]
    return module_name or "model"


def _extract_file_refs(schema: Dict, seen: set = None) -> set:
    """
    Extract all file $ref references from a schema.
    Returns set of referenced filenames like {'Bond.json', 'Address.json'}
    """
    if seen is None:
        seen = set()
    ...
    if isinstance(schema, dict):
        if "$ref" in schema:
            ref = schema["$ref"]
            # Only include file references (not internal like #/definitions/...)
            if not ref.startswith("#") and not ref.startswith("http"):
                # Extract just the filename
                ref_file = Path(ref).name
                refs.add(ref_file)
        
        for key, value in schema.items():
            if key != "$ref":
                refs.update(_extract_file_refs(value, seen))
    
    elif isinstance(schema, list):
        for item in schema:
            refs.update(_extract_file_refs(item, seen))
...
class ModuleGenerator:
    ...
    def generate(self) -> Dict[str, Any]:
        ...
        # === FIRST PASS: Build mapping of schema files to class/module names ===
        schema_info = {}  # filename -> (schema, class_name, module_name)
        for schema_file in sorted(self.schema_dir.glob("*.json")):
            try:
                ...
                schema_info[schema_file.name] = (schema, class_name, module_name, schema_file)
        ...
        # Build reverse mapping: filename -> class_name
        file_to_class = {fname: info[1] for fname, info in schema_info.items()}
        
        # === SECOND PASS: Generate code with proper imports ===
        for filename, (schema, class_name, module_name, schema_file) in schema_info.items():
            try:
                ...
                # Find $ref references to other schema files
                refs = _extract_file_refs(schema)
                
                # Build import statements for referenced classes
                import_lines = []
                for ref_file in sorted(refs):
                    if ref_file in file_to_class:
                        ref_class = file_to_class[ref_file]
                        ref_module = self.class_to_module.get(ref_class, to_snake_case(ref_class))
                        # Don't import self
                        if ref_class != class_name:
                            import_lines.append(f"from .{ref_module} import {ref_class}")
                ...
                # Add imports for referenced classes after the standard imports
                if import_lines:
                    # Find where to insert (after standard imports)
                    lines = full_code.split('\n')
                    insert_idx = 0
                    for i, line in enumerate(lines):
                        if line.startswith('from datetime') or line.startswith('from uuid'):
                            insert_idx = i + 1
                        elif line.startswith('from typing'):
                            insert_idx = i + 1
                        elif line.startswith('from dataclasses'):
                            insert_idx = i + 1
                    ...
                    import_block = '\n# Imports for referenced schema classes\n' + '\n'.join(import_lines)
