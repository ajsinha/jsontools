from __future__ import annotations

"""
Auto-generated by JsonSchemaCodeGen v1.2.0
Generated: 2026-01-29T14:57:05.100163

Copyright (C) 2025-2030, Ashutosh Sinha (ajsinha@gmail.com)
DO NOT EDIT - Changes will be overwritten.
"""
# Generated from: 11_healthcare_patient.json

"""
Auto-generated Python classes from JSON Schema.

Generated by JsonSchemaCodeGen on 2026-01-29 14:57:05

This module contains dataclasses generated from JSON Schema definitions.
Each class includes:
- Type hints for all properties
- Serialization methods (to_dict, to_json)
- Deserialization class methods (from_dict, from_json)
- Validation method to check required fields and enum values
"""


import json
from dataclasses import dataclass, field, asdict
from typing import Any, Dict, List, Optional, Union
from datetime import datetime, date
from typing import List
from uuid import UUID


@dataclass
class ValidationResult:
    """Result of validation containing status and any errors."""
    is_valid: bool
    errors: List[str] = field(default_factory=list)
    
    def __str__(self) -> str:
        if self.is_valid:
            return "Valid"
        return f"Invalid: {'; '.join(self.errors)}"
    
    def __bool__(self) -> bool:
        return self.is_valid

@dataclass
class Name:
    """Generated class for Name."""

    # Property name mapping for JSON serialization
    @classmethod
    def _get_json_mapping(cls) -> Dict[str, str]:
        return {
            "family": "family",
            "given": "given",
            "prefix": "prefix",
            "suffix": "suffix"
        }

    family: Optional[str] = None
    given: List[str] = field(default_factory=list)
    prefix: Optional[str] = None
    suffix: Optional[str] = None

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary with original JSON property names."""
        result = {}
        for py_name, json_name in self._get_json_mapping().items():
            value = getattr(self, py_name, None)
            if value is not None:
                if hasattr(value, 'to_dict'):
                    result[json_name] = value.to_dict()
                elif isinstance(value, list):
                    result[json_name] = [
                        item.to_dict() if hasattr(item, 'to_dict') else item
                        for item in value
                    ]
                elif isinstance(value, (datetime, date)):
                    result[json_name] = value.isoformat()
                else:
                    result[json_name] = value
        return result

    def to_json(self, indent: int = 2) -> str:
        """Serialize to JSON string."""
        return json.dumps(self.to_dict(), indent=indent, default=str)

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'Name':
        """Create instance from dictionary."""
        reverse_mapping = {v: k for k, v in cls._get_json_mapping().items()}
        kwargs = {}
        for json_name, value in data.items():
            py_name = reverse_mapping.get(json_name, json_name)
            if py_name in cls.__annotations__:
                kwargs[py_name] = value
        return cls(**kwargs)

    @classmethod
    def from_json(cls, json_str: str) -> 'Name':
        """Create instance from JSON string."""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def _get_required_fields(cls) -> List[str]:
        """Get list of required field names."""
        return ['family']

    @classmethod
    def _get_enum_constraints(cls) -> Dict[str, List[Any]]:
        """Get enum constraints for fields."""
        return {}

    def validate(self) -> 'ValidationResult':
        """
        Validate that all required fields are populated and enum values are valid.
        
        Returns:
            ValidationResult with is_valid flag and list of errors
        """
        errors = []
        
        # Check required fields
        for field_name in self._get_required_fields():
            value = getattr(self, field_name, None)
            if value is None:
                errors.append(f"Required field '{field_name}' is not set")
        
        # Check enum constraints
        for field_name, valid_values in self._get_enum_constraints().items():
            value = getattr(self, field_name, None)
            if value is not None and value not in valid_values:
                errors.append(f"Field '{field_name}' has invalid value '{value}'. Must be one of: {valid_values}")
        
        return ValidationResult(is_valid=len(errors) == 0, errors=errors)

@dataclass
class ContactPoint:
    """Generated class for ContactPoint."""

    # Property name mapping for JSON serialization
    @classmethod
    def _get_json_mapping(cls) -> Dict[str, str]:
        return {
            "system": "system",
            "value": "value",
            "use": "use",
            "rank": "rank"
        }

    system: Optional["SystemEnum"] = None
    value: Optional[str] = None
    use: Optional["UseEnum"] = None
    rank: Optional[int] = None

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary with original JSON property names."""
        result = {}
        for py_name, json_name in self._get_json_mapping().items():
            value = getattr(self, py_name, None)
            if value is not None:
                if hasattr(value, 'to_dict'):
                    result[json_name] = value.to_dict()
                elif isinstance(value, list):
                    result[json_name] = [
                        item.to_dict() if hasattr(item, 'to_dict') else item
                        for item in value
                    ]
                elif isinstance(value, (datetime, date)):
                    result[json_name] = value.isoformat()
                else:
                    result[json_name] = value
        return result

    def to_json(self, indent: int = 2) -> str:
        """Serialize to JSON string."""
        return json.dumps(self.to_dict(), indent=indent, default=str)

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'ContactPoint':
        """Create instance from dictionary."""
        reverse_mapping = {v: k for k, v in cls._get_json_mapping().items()}
        kwargs = {}
        for json_name, value in data.items():
            py_name = reverse_mapping.get(json_name, json_name)
            if py_name in cls.__annotations__:
                kwargs[py_name] = value
        return cls(**kwargs)

    @classmethod
    def from_json(cls, json_str: str) -> 'ContactPoint':
        """Create instance from JSON string."""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def _get_required_fields(cls) -> List[str]:
        """Get list of required field names."""
        return ['system', 'value']

    @classmethod
    def _get_enum_constraints(cls) -> Dict[str, List[Any]]:
        """Get enum constraints for fields."""
        return {'system': ['phone', 'email', 'fax', 'url'], 'use': ['home', 'work', 'mobile']}

    def validate(self) -> 'ValidationResult':
        """
        Validate that all required fields are populated and enum values are valid.
        
        Returns:
            ValidationResult with is_valid flag and list of errors
        """
        errors = []
        
        # Check required fields
        for field_name in self._get_required_fields():
            value = getattr(self, field_name, None)
            if value is None:
                errors.append(f"Required field '{field_name}' is not set")
        
        # Check enum constraints
        for field_name, valid_values in self._get_enum_constraints().items():
            value = getattr(self, field_name, None)
            if value is not None and value not in valid_values:
                errors.append(f"Field '{field_name}' has invalid value '{value}'. Must be one of: {valid_values}")
        
        return ValidationResult(is_valid=len(errors) == 0, errors=errors)

@dataclass
class Address:
    """Generated class for Address."""

    # Property name mapping for JSON serialization
    @classmethod
    def _get_json_mapping(cls) -> Dict[str, str]:
        return {
            "use": "use",
            "line": "line",
            "city": "city",
            "state": "state",
            "postalCode": "postalCode",
            "country": "country"
        }

    use: Optional["UseEnum"] = None
    line: List[str] = field(default_factory=list)
    city: Optional[str] = None
    state: Optional[str] = None
    postalCode: Optional[str] = None
    country: Optional[str] = None

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary with original JSON property names."""
        result = {}
        for py_name, json_name in self._get_json_mapping().items():
            value = getattr(self, py_name, None)
            if value is not None:
                if hasattr(value, 'to_dict'):
                    result[json_name] = value.to_dict()
                elif isinstance(value, list):
                    result[json_name] = [
                        item.to_dict() if hasattr(item, 'to_dict') else item
                        for item in value
                    ]
                elif isinstance(value, (datetime, date)):
                    result[json_name] = value.isoformat()
                else:
                    result[json_name] = value
        return result

    def to_json(self, indent: int = 2) -> str:
        """Serialize to JSON string."""
        return json.dumps(self.to_dict(), indent=indent, default=str)

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'Address':
        """Create instance from dictionary."""
        reverse_mapping = {v: k for k, v in cls._get_json_mapping().items()}
        kwargs = {}
        for json_name, value in data.items():
            py_name = reverse_mapping.get(json_name, json_name)
            if py_name in cls.__annotations__:
                kwargs[py_name] = value
        return cls(**kwargs)

    @classmethod
    def from_json(cls, json_str: str) -> 'Address':
        """Create instance from JSON string."""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def _get_required_fields(cls) -> List[str]:
        """Get list of required field names."""
        return []

    @classmethod
    def _get_enum_constraints(cls) -> Dict[str, List[Any]]:
        """Get enum constraints for fields."""
        return {'use': ['home', 'work', 'temp', 'billing']}

    def validate(self) -> 'ValidationResult':
        """
        Validate that all required fields are populated and enum values are valid.
        
        Returns:
            ValidationResult with is_valid flag and list of errors
        """
        errors = []
        
        # Check required fields
        for field_name in self._get_required_fields():
            value = getattr(self, field_name, None)
            if value is None:
                errors.append(f"Required field '{field_name}' is not set")
        
        # Check enum constraints
        for field_name, valid_values in self._get_enum_constraints().items():
            value = getattr(self, field_name, None)
            if value is not None and value not in valid_values:
                errors.append(f"Field '{field_name}' has invalid value '{value}'. Must be one of: {valid_values}")
        
        return ValidationResult(is_valid=len(errors) == 0, errors=errors)

@dataclass
class Identifier:
    """Generated class for Identifier."""

    # Property name mapping for JSON serialization
    @classmethod
    def _get_json_mapping(cls) -> Dict[str, str]:
        return {
            "value": "value",
            "system": "system",
            "type_": "type"
        }

    value: Optional[str] = None
    system: Optional[str] = None
    type_: Optional[str] = None

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary with original JSON property names."""
        result = {}
        for py_name, json_name in self._get_json_mapping().items():
            value = getattr(self, py_name, None)
            if value is not None:
                if hasattr(value, 'to_dict'):
                    result[json_name] = value.to_dict()
                elif isinstance(value, list):
                    result[json_name] = [
                        item.to_dict() if hasattr(item, 'to_dict') else item
                        for item in value
                    ]
                elif isinstance(value, (datetime, date)):
                    result[json_name] = value.isoformat()
                else:
                    result[json_name] = value
        return result

    def to_json(self, indent: int = 2) -> str:
        """Serialize to JSON string."""
        return json.dumps(self.to_dict(), indent=indent, default=str)

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'Identifier':
        """Create instance from dictionary."""
        reverse_mapping = {v: k for k, v in cls._get_json_mapping().items()}
        kwargs = {}
        for json_name, value in data.items():
            py_name = reverse_mapping.get(json_name, json_name)
            if py_name in cls.__annotations__:
                kwargs[py_name] = value
        return cls(**kwargs)

    @classmethod
    def from_json(cls, json_str: str) -> 'Identifier':
        """Create instance from JSON string."""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def _get_required_fields(cls) -> List[str]:
        """Get list of required field names."""
        return ['value']

    @classmethod
    def _get_enum_constraints(cls) -> Dict[str, List[Any]]:
        """Get enum constraints for fields."""
        return {}

    def validate(self) -> 'ValidationResult':
        """
        Validate that all required fields are populated and enum values are valid.
        
        Returns:
            ValidationResult with is_valid flag and list of errors
        """
        errors = []
        
        # Check required fields
        for field_name in self._get_required_fields():
            value = getattr(self, field_name, None)
            if value is None:
                errors.append(f"Required field '{field_name}' is not set")
        
        # Check enum constraints
        for field_name, valid_values in self._get_enum_constraints().items():
            value = getattr(self, field_name, None)
            if value is not None and value not in valid_values:
                errors.append(f"Field '{field_name}' has invalid value '{value}'. Must be one of: {valid_values}")
        
        return ValidationResult(is_valid=len(errors) == 0, errors=errors)

@dataclass
class EmergencyContact:
    """Generated class for EmergencyContact."""

    # Property name mapping for JSON serialization
    @classmethod
    def _get_json_mapping(cls) -> Dict[str, str]:
        return {
            "relationship": "relationship",
            "name": "name",
            "telecom": "telecom",
            "address": "address"
        }

    relationship: Optional[str] = None
    name: Optional["Name"] = None
    telecom: List["AnonymousObject"] = field(default_factory=list)
    address: Optional["Address"] = None

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary with original JSON property names."""
        result = {}
        for py_name, json_name in self._get_json_mapping().items():
            value = getattr(self, py_name, None)
            if value is not None:
                if hasattr(value, 'to_dict'):
                    result[json_name] = value.to_dict()
                elif isinstance(value, list):
                    result[json_name] = [
                        item.to_dict() if hasattr(item, 'to_dict') else item
                        for item in value
                    ]
                elif isinstance(value, (datetime, date)):
                    result[json_name] = value.isoformat()
                else:
                    result[json_name] = value
        return result

    def to_json(self, indent: int = 2) -> str:
        """Serialize to JSON string."""
        return json.dumps(self.to_dict(), indent=indent, default=str)

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'EmergencyContact':
        """Create instance from dictionary."""
        reverse_mapping = {v: k for k, v in cls._get_json_mapping().items()}
        kwargs = {}
        for json_name, value in data.items():
            py_name = reverse_mapping.get(json_name, json_name)
            if py_name in cls.__annotations__:
                kwargs[py_name] = value
        return cls(**kwargs)

    @classmethod
    def from_json(cls, json_str: str) -> 'EmergencyContact':
        """Create instance from JSON string."""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def _get_required_fields(cls) -> List[str]:
        """Get list of required field names."""
        return []

    @classmethod
    def _get_enum_constraints(cls) -> Dict[str, List[Any]]:
        """Get enum constraints for fields."""
        return {}

    def validate(self) -> 'ValidationResult':
        """
        Validate that all required fields are populated and enum values are valid.
        
        Returns:
            ValidationResult with is_valid flag and list of errors
        """
        errors = []
        
        # Check required fields
        for field_name in self._get_required_fields():
            value = getattr(self, field_name, None)
            if value is None:
                errors.append(f"Required field '{field_name}' is not set")
        
        # Check enum constraints
        for field_name, valid_values in self._get_enum_constraints().items():
            value = getattr(self, field_name, None)
            if value is not None and value not in valid_values:
                errors.append(f"Field '{field_name}' has invalid value '{value}'. Must be one of: {valid_values}")
        
        return ValidationResult(is_valid=len(errors) == 0, errors=errors)

@dataclass
class Insurance:
    """Generated class for Insurance."""

    # Property name mapping for JSON serialization
    @classmethod
    def _get_json_mapping(cls) -> Dict[str, str]:
        return {
            "provider": "provider",
            "policyNumber": "policyNumber",
            "groupNumber": "groupNumber",
            "subscriberId": "subscriberId",
            "effectiveDate": "effectiveDate",
            "expirationDate": "expirationDate"
        }

    provider: Optional[str] = None
    policyNumber: Optional[str] = None
    groupNumber: Optional[str] = None
    subscriberId: Optional[str] = None
    effectiveDate: Optional[date] = None
    expirationDate: Optional[date] = None

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary with original JSON property names."""
        result = {}
        for py_name, json_name in self._get_json_mapping().items():
            value = getattr(self, py_name, None)
            if value is not None:
                if hasattr(value, 'to_dict'):
                    result[json_name] = value.to_dict()
                elif isinstance(value, list):
                    result[json_name] = [
                        item.to_dict() if hasattr(item, 'to_dict') else item
                        for item in value
                    ]
                elif isinstance(value, (datetime, date)):
                    result[json_name] = value.isoformat()
                else:
                    result[json_name] = value
        return result

    def to_json(self, indent: int = 2) -> str:
        """Serialize to JSON string."""
        return json.dumps(self.to_dict(), indent=indent, default=str)

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'Insurance':
        """Create instance from dictionary."""
        reverse_mapping = {v: k for k, v in cls._get_json_mapping().items()}
        kwargs = {}
        for json_name, value in data.items():
            py_name = reverse_mapping.get(json_name, json_name)
            if py_name in cls.__annotations__:
                kwargs[py_name] = value
        return cls(**kwargs)

    @classmethod
    def from_json(cls, json_str: str) -> 'Insurance':
        """Create instance from JSON string."""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def _get_required_fields(cls) -> List[str]:
        """Get list of required field names."""
        return ['provider', 'policyNumber']

    @classmethod
    def _get_enum_constraints(cls) -> Dict[str, List[Any]]:
        """Get enum constraints for fields."""
        return {}

    def validate(self) -> 'ValidationResult':
        """
        Validate that all required fields are populated and enum values are valid.
        
        Returns:
            ValidationResult with is_valid flag and list of errors
        """
        errors = []
        
        # Check required fields
        for field_name in self._get_required_fields():
            value = getattr(self, field_name, None)
            if value is None:
                errors.append(f"Required field '{field_name}' is not set")
        
        # Check enum constraints
        for field_name, valid_values in self._get_enum_constraints().items():
            value = getattr(self, field_name, None)
            if value is not None and value not in valid_values:
                errors.append(f"Field '{field_name}' has invalid value '{value}'. Must be one of: {valid_values}")
        
        return ValidationResult(is_valid=len(errors) == 0, errors=errors)

@dataclass
class Patient:
    """Healthcare patient record"""

    # Property name mapping for JSON serialization
    @classmethod
    def _get_json_mapping(cls) -> Dict[str, str]:
        return {
            "id_": "id",
            "name": "name",
            "birthDate": "birthDate",
            "identifiers": "identifiers",
            "active": "active",
            "telecom": "telecom",
            "gender": "gender",
            "deceasedBoolean": "deceasedBoolean",
            "deceasedDateTime": "deceasedDateTime",
            "address": "address",
            "maritalStatus": "maritalStatus",
            "emergencyContacts": "emergencyContacts",
            "insurance": "insurance",
            "generalPractitioner": "generalPractitioner",
            "preferredLanguage": "preferredLanguage",
            "createdAt": "createdAt",
            "updatedAt": "updatedAt"
        }

    id_: Optional[UUID] = None
    name: List["AnonymousObject"] = field(default_factory=list)
    birthDate: Optional[date] = None
    identifiers: List["AnonymousObject"] = field(default_factory=list)
    active: Optional[bool] = True
    telecom: List["AnonymousObject"] = field(default_factory=list)
    gender: Optional["GenderEnum"] = None
    deceasedBoolean: Optional[bool] = None
    deceasedDateTime: Optional[datetime] = None
    address: List["AnonymousObject"] = field(default_factory=list)
    maritalStatus: Optional["MaritalstatusEnum"] = None
    emergencyContacts: List["AnonymousObject"] = field(default_factory=list)
    insurance: List["AnonymousObject"] = field(default_factory=list)
    generalPractitioner: Optional[str] = None
    preferredLanguage: Optional[str] = None
    createdAt: Optional[datetime] = None
    updatedAt: Optional[datetime] = None

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary with original JSON property names."""
        result = {}
        for py_name, json_name in self._get_json_mapping().items():
            value = getattr(self, py_name, None)
            if value is not None:
                if hasattr(value, 'to_dict'):
                    result[json_name] = value.to_dict()
                elif isinstance(value, list):
                    result[json_name] = [
                        item.to_dict() if hasattr(item, 'to_dict') else item
                        for item in value
                    ]
                elif isinstance(value, (datetime, date)):
                    result[json_name] = value.isoformat()
                else:
                    result[json_name] = value
        return result

    def to_json(self, indent: int = 2) -> str:
        """Serialize to JSON string."""
        return json.dumps(self.to_dict(), indent=indent, default=str)

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'Patient':
        """Create instance from dictionary."""
        reverse_mapping = {v: k for k, v in cls._get_json_mapping().items()}
        kwargs = {}
        for json_name, value in data.items():
            py_name = reverse_mapping.get(json_name, json_name)
            if py_name in cls.__annotations__:
                kwargs[py_name] = value
        return cls(**kwargs)

    @classmethod
    def from_json(cls, json_str: str) -> 'Patient':
        """Create instance from JSON string."""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def _get_required_fields(cls) -> List[str]:
        """Get list of required field names."""
        return ['id_', 'name', 'birthDate']

    @classmethod
    def _get_enum_constraints(cls) -> Dict[str, List[Any]]:
        """Get enum constraints for fields."""
        return {'gender': ['male', 'female', 'other', 'unknown'], 'maritalStatus': ['single', 'married', 'divorced', 'widowed']}

    def validate(self) -> 'ValidationResult':
        """
        Validate that all required fields are populated and enum values are valid.
        
        Returns:
            ValidationResult with is_valid flag and list of errors
        """
        errors = []
        
        # Check required fields
        for field_name in self._get_required_fields():
            value = getattr(self, field_name, None)
            if value is None:
                errors.append(f"Required field '{field_name}' is not set")
        
        # Check enum constraints
        for field_name, valid_values in self._get_enum_constraints().items():
            value = getattr(self, field_name, None)
            if value is not None and value not in valid_values:
                errors.append(f"Field '{field_name}' has invalid value '{value}'. Must be one of: {valid_values}")
        
        return ValidationResult(is_valid=len(errors) == 0, errors=errors)
