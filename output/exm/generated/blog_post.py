from __future__ import annotations

"""
Auto-generated by JsonSchemaCodeGen v1.2.0
Generated: 2026-01-29T14:57:05.094435

Copyright (C) 2025-2030, Ashutosh Sinha (ajsinha@gmail.com)
DO NOT EDIT - Changes will be overwritten.
"""
# Generated from: 05_blog_post.json

"""
Auto-generated Python classes from JSON Schema.

Generated by JsonSchemaCodeGen on 2026-01-29 14:57:05

This module contains dataclasses generated from JSON Schema definitions.
Each class includes:
- Type hints for all properties
- Serialization methods (to_dict, to_json)
- Deserialization class methods (from_dict, from_json)
- Validation method to check required fields and enum values
"""


import json
from dataclasses import dataclass, field, asdict
from typing import Any, Dict, List, Optional, Union
from datetime import datetime, date
from typing import List
from uuid import UUID


@dataclass
class ValidationResult:
    """Result of validation containing status and any errors."""
    is_valid: bool
    errors: List[str] = field(default_factory=list)
    
    def __str__(self) -> str:
        if self.is_valid:
            return "Valid"
        return f"Invalid: {'; '.join(self.errors)}"
    
    def __bool__(self) -> bool:
        return self.is_valid

@dataclass
class Author:
    """Generated class for Author."""

    # Property name mapping for JSON serialization
    @classmethod
    def _get_json_mapping(cls) -> Dict[str, str]:
        return {
            "id_": "id",
            "name": "name",
            "email": "email",
            "bio": "bio",
            "avatar": "avatar"
        }

    id_: Optional[UUID] = None
    name: Optional[str] = None
    email: Optional[str] = None
    bio: Optional[str] = None
    avatar: Optional[str] = None

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary with original JSON property names."""
        result = {}
        for py_name, json_name in self._get_json_mapping().items():
            value = getattr(self, py_name, None)
            if value is not None:
                if hasattr(value, 'to_dict'):
                    result[json_name] = value.to_dict()
                elif isinstance(value, list):
                    result[json_name] = [
                        item.to_dict() if hasattr(item, 'to_dict') else item
                        for item in value
                    ]
                elif isinstance(value, (datetime, date)):
                    result[json_name] = value.isoformat()
                else:
                    result[json_name] = value
        return result

    def to_json(self, indent: int = 2) -> str:
        """Serialize to JSON string."""
        return json.dumps(self.to_dict(), indent=indent, default=str)

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'Author':
        """Create instance from dictionary."""
        reverse_mapping = {v: k for k, v in cls._get_json_mapping().items()}
        kwargs = {}
        for json_name, value in data.items():
            py_name = reverse_mapping.get(json_name, json_name)
            if py_name in cls.__annotations__:
                kwargs[py_name] = value
        return cls(**kwargs)

    @classmethod
    def from_json(cls, json_str: str) -> 'Author':
        """Create instance from JSON string."""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def _get_required_fields(cls) -> List[str]:
        """Get list of required field names."""
        return ['id_', 'name']

    @classmethod
    def _get_enum_constraints(cls) -> Dict[str, List[Any]]:
        """Get enum constraints for fields."""
        return {}

    def validate(self) -> 'ValidationResult':
        """
        Validate that all required fields are populated and enum values are valid.
        
        Returns:
            ValidationResult with is_valid flag and list of errors
        """
        errors = []
        
        # Check required fields
        for field_name in self._get_required_fields():
            value = getattr(self, field_name, None)
            if value is None:
                errors.append(f"Required field '{field_name}' is not set")
        
        # Check enum constraints
        for field_name, valid_values in self._get_enum_constraints().items():
            value = getattr(self, field_name, None)
            if value is not None and value not in valid_values:
                errors.append(f"Field '{field_name}' has invalid value '{value}'. Must be one of: {valid_values}")
        
        return ValidationResult(is_valid=len(errors) == 0, errors=errors)

@dataclass
class Comment:
    """Generated class for Comment."""

    # Property name mapping for JSON serialization
    @classmethod
    def _get_json_mapping(cls) -> Dict[str, str]:
        return {
            "id_": "id",
            "author": "author",
            "content": "content",
            "createdAt": "createdAt",
            "parentId": "parentId",
            "replies": "replies",
            "likes": "likes"
        }

    id_: Optional[UUID] = None
    author: Optional["Author"] = None
    content: Optional[str] = None
    createdAt: Optional[datetime] = None
    parentId: Optional[UUID] = None
    replies: List["AnonymousObject"] = field(default_factory=list)
    likes: Optional[int] = None

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary with original JSON property names."""
        result = {}
        for py_name, json_name in self._get_json_mapping().items():
            value = getattr(self, py_name, None)
            if value is not None:
                if hasattr(value, 'to_dict'):
                    result[json_name] = value.to_dict()
                elif isinstance(value, list):
                    result[json_name] = [
                        item.to_dict() if hasattr(item, 'to_dict') else item
                        for item in value
                    ]
                elif isinstance(value, (datetime, date)):
                    result[json_name] = value.isoformat()
                else:
                    result[json_name] = value
        return result

    def to_json(self, indent: int = 2) -> str:
        """Serialize to JSON string."""
        return json.dumps(self.to_dict(), indent=indent, default=str)

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'Comment':
        """Create instance from dictionary."""
        reverse_mapping = {v: k for k, v in cls._get_json_mapping().items()}
        kwargs = {}
        for json_name, value in data.items():
            py_name = reverse_mapping.get(json_name, json_name)
            if py_name in cls.__annotations__:
                kwargs[py_name] = value
        return cls(**kwargs)

    @classmethod
    def from_json(cls, json_str: str) -> 'Comment':
        """Create instance from JSON string."""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def _get_required_fields(cls) -> List[str]:
        """Get list of required field names."""
        return ['id_', 'author', 'content', 'createdAt']

    @classmethod
    def _get_enum_constraints(cls) -> Dict[str, List[Any]]:
        """Get enum constraints for fields."""
        return {}

    def validate(self) -> 'ValidationResult':
        """
        Validate that all required fields are populated and enum values are valid.
        
        Returns:
            ValidationResult with is_valid flag and list of errors
        """
        errors = []
        
        # Check required fields
        for field_name in self._get_required_fields():
            value = getattr(self, field_name, None)
            if value is None:
                errors.append(f"Required field '{field_name}' is not set")
        
        # Check enum constraints
        for field_name, valid_values in self._get_enum_constraints().items():
            value = getattr(self, field_name, None)
            if value is not None and value not in valid_values:
                errors.append(f"Field '{field_name}' has invalid value '{value}'. Must be one of: {valid_values}")
        
        return ValidationResult(is_valid=len(errors) == 0, errors=errors)

@dataclass
class Tag:
    """Generated class for Tag."""

    # Property name mapping for JSON serialization
    @classmethod
    def _get_json_mapping(cls) -> Dict[str, str]:
        return {
            "id_": "id",
            "name": "name",
            "slug": "slug"
        }

    id_: Optional[str] = None
    name: Optional[str] = None
    slug: Optional[str] = None

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary with original JSON property names."""
        result = {}
        for py_name, json_name in self._get_json_mapping().items():
            value = getattr(self, py_name, None)
            if value is not None:
                if hasattr(value, 'to_dict'):
                    result[json_name] = value.to_dict()
                elif isinstance(value, list):
                    result[json_name] = [
                        item.to_dict() if hasattr(item, 'to_dict') else item
                        for item in value
                    ]
                elif isinstance(value, (datetime, date)):
                    result[json_name] = value.isoformat()
                else:
                    result[json_name] = value
        return result

    def to_json(self, indent: int = 2) -> str:
        """Serialize to JSON string."""
        return json.dumps(self.to_dict(), indent=indent, default=str)

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'Tag':
        """Create instance from dictionary."""
        reverse_mapping = {v: k for k, v in cls._get_json_mapping().items()}
        kwargs = {}
        for json_name, value in data.items():
            py_name = reverse_mapping.get(json_name, json_name)
            if py_name in cls.__annotations__:
                kwargs[py_name] = value
        return cls(**kwargs)

    @classmethod
    def from_json(cls, json_str: str) -> 'Tag':
        """Create instance from JSON string."""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def _get_required_fields(cls) -> List[str]:
        """Get list of required field names."""
        return ['id_', 'name', 'slug']

    @classmethod
    def _get_enum_constraints(cls) -> Dict[str, List[Any]]:
        """Get enum constraints for fields."""
        return {}

    def validate(self) -> 'ValidationResult':
        """
        Validate that all required fields are populated and enum values are valid.
        
        Returns:
            ValidationResult with is_valid flag and list of errors
        """
        errors = []
        
        # Check required fields
        for field_name in self._get_required_fields():
            value = getattr(self, field_name, None)
            if value is None:
                errors.append(f"Required field '{field_name}' is not set")
        
        # Check enum constraints
        for field_name, valid_values in self._get_enum_constraints().items():
            value = getattr(self, field_name, None)
            if value is not None and value not in valid_values:
                errors.append(f"Field '{field_name}' has invalid value '{value}'. Must be one of: {valid_values}")
        
        return ValidationResult(is_valid=len(errors) == 0, errors=errors)

@dataclass
class BlogPost:
    """Blog post with comments and metadata"""

    # Property name mapping for JSON serialization
    @classmethod
    def _get_json_mapping(cls) -> Dict[str, str]:
        return {
            "id_": "id",
            "title": "title",
            "slug": "slug",
            "content": "content",
            "author": "author",
            "status": "status",
            "excerpt": "excerpt",
            "contentFormat": "contentFormat",
            "featuredImage": "featuredImage",
            "coAuthors": "coAuthors",
            "tags": "tags",
            "category": "category",
            "comments": "comments",
            "commentCount": "commentCount",
            "viewCount": "viewCount",
            "likeCount": "likeCount",
            "publishedAt": "publishedAt",
            "createdAt": "createdAt",
            "updatedAt": "updatedAt"
        }

    id_: Optional[UUID] = None
    title: Optional[str] = None
    slug: Optional[str] = None
    content: Optional[str] = None
    author: Optional["Author"] = None
    status: Optional["StatusEnum"] = None
    excerpt: Optional[str] = None
    contentFormat: Optional["ContentformatEnum"] = None
    featuredImage: Optional[str] = None
    coAuthors: List["AnonymousObject"] = field(default_factory=list)
    tags: List["AnonymousObject"] = field(default_factory=list)
    category: Optional[str] = None
    comments: List["AnonymousObject"] = field(default_factory=list)
    commentCount: Optional[int] = None
    viewCount: Optional[int] = None
    likeCount: Optional[int] = None
    publishedAt: Optional[datetime] = None
    createdAt: Optional[datetime] = None
    updatedAt: Optional[datetime] = None

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary with original JSON property names."""
        result = {}
        for py_name, json_name in self._get_json_mapping().items():
            value = getattr(self, py_name, None)
            if value is not None:
                if hasattr(value, 'to_dict'):
                    result[json_name] = value.to_dict()
                elif isinstance(value, list):
                    result[json_name] = [
                        item.to_dict() if hasattr(item, 'to_dict') else item
                        for item in value
                    ]
                elif isinstance(value, (datetime, date)):
                    result[json_name] = value.isoformat()
                else:
                    result[json_name] = value
        return result

    def to_json(self, indent: int = 2) -> str:
        """Serialize to JSON string."""
        return json.dumps(self.to_dict(), indent=indent, default=str)

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'BlogPost':
        """Create instance from dictionary."""
        reverse_mapping = {v: k for k, v in cls._get_json_mapping().items()}
        kwargs = {}
        for json_name, value in data.items():
            py_name = reverse_mapping.get(json_name, json_name)
            if py_name in cls.__annotations__:
                kwargs[py_name] = value
        return cls(**kwargs)

    @classmethod
    def from_json(cls, json_str: str) -> 'BlogPost':
        """Create instance from JSON string."""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def _get_required_fields(cls) -> List[str]:
        """Get list of required field names."""
        return ['id_', 'title', 'slug', 'content', 'author', 'status']

    @classmethod
    def _get_enum_constraints(cls) -> Dict[str, List[Any]]:
        """Get enum constraints for fields."""
        return {'status': ['draft', 'published', 'archived'], 'contentFormat': ['markdown', 'html', 'plain']}

    def validate(self) -> 'ValidationResult':
        """
        Validate that all required fields are populated and enum values are valid.
        
        Returns:
            ValidationResult with is_valid flag and list of errors
        """
        errors = []
        
        # Check required fields
        for field_name in self._get_required_fields():
            value = getattr(self, field_name, None)
            if value is None:
                errors.append(f"Required field '{field_name}' is not set")
        
        # Check enum constraints
        for field_name, valid_values in self._get_enum_constraints().items():
            value = getattr(self, field_name, None)
            if value is not None and value not in valid_values:
                errors.append(f"Field '{field_name}' has invalid value '{value}'. Must be one of: {valid_values}")
        
        return ValidationResult(is_valid=len(errors) == 0, errors=errors)
